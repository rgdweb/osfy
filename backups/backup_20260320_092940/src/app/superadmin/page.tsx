    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="border-slate-200">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Faturas Pendentes</CardTitle>
            <Calendar className="h-4 w-4 text-amber-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-amber-600">{stats.faturasPendentes}</div>
            <p className="text-xs text-slate-500">aguardando pagamento</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-slate-200">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{stats.faturasVencidas}</div>
        ...
        <Card className="border-slate-200">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Valor em Aberto</CardTitle>
            <DollarSign className="h-4 w-4 text-amber-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-amber-600">
              {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(stats.valorEmAberto || 0)}
            </div>
        ...
        <Card className="border-slate-200">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(stats.receitaMensal || 0)}
            </div>
            <p className="text-xs text-slate-500">faturas pagas</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          ...
          <div className="text-slate-500">Faturamento Total (OS)</div>
        </div>
      </div>

      {/* Lojas Inadimplentes */}
      {stats.lojasInadimplentes && stats.lojasInadimplentes.length > 0 && (
        <div className="bg-white rounded-xl border border-red-200 overflow-hidden">
          <div className="p-6 border-b border-red-200 flex items-center justify-between bg-red-50">
            <div className="flex items-center gap-2">
              <AlertTriangle className="w-5 h-5 text-red-600" />
              <h2 className="text-lg font-semibold text-red-800">Lojas Inadimplentes</h2>
            </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Faturas Vencidas</th>
              ...
              <tbody className="divide-y divide-slate-200">
                {stats.lojasInadimplentes.map((loja) => (
                  <tr key={loja.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4">
                      <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-700">
                        {loja.totalFaturasVencidas} fatura{loja.totalFaturasVencidas > 1 ? 's' : ''}
                      </span>
                    </td>
                    <td className="px-6 py-4 font-medium text-red-600">
                      {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(loja.valorTotal)}
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        loja.status === 'bloqueada' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
                      }`}>
                        {loja.status}
                      </span>
                    </td>

  ...
  try {
    ...
    if (busca) {
      where.OR = [
        ...
        { cliente: { telefone: { contains: busca, mode: 'insensitive' } } },
...
export async function POST(request: NextRequest) {
  try {
    ...
    // === LIMITE DE 100 OS POR LOJA ===
    // Contar OS atuais da loja
    const totalOS = await db.ordemServico.count({
      where: { lojaId: user.lojaId }
    })

    // Se já tem 100 ou mais, deletar a mais antiga
    if (totalOS >= 100) {
      const osAntiga = await db.ordemServico.findFirst({
        where: { lojaId: user.lojaId },
      ...
      if (osAntiga) {
        // Deletar OS antiga (cascade deleta fotos, histórico, assinatura)
        await db.ordemServico.delete({
          where: { id: osAntiga.id }
        })
      }
    }
    // === FIM DO LIMITE ===

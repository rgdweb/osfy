...
export default function FrenteDeCaixa() {
  ...
  // Listener para código de barras via keyboard (detecta automaticamente pela velocidade)
  useEffect(() => {
    let timeoutId: NodeJS.Timeout
    let lastKeyTime = 0
    let inputBuffer = ''
    let isFastTyping = false
    const FAST_TYPING_THRESHOLD = 30 // ms entre teclas (leitor é muito rápido)

    const handleKeyDown = (e: KeyboardEvent) => {
      const now = Date.now()
      const isInput = e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement

      // F2 abre busca de produtos (sempre funciona)
      if (e.key === 'F2') {
        e.preventDefault()
      ...
      // F3 abre venda avulsa (sempre funciona)
      if (e.key === 'F3') {
        e.preventDefault()
      ...
      // F4 abre pagamento (sempre funciona)
      if (e.key === 'F4') {
        e.preventDefault()
        if (itens.length > 0) {
      ...
      // Se está em um input/textarea, usa detecção por velocidade
      if (isInput) {
        const target = e.target as HTMLInputElement | HTMLTextAreaElement

        // Enter pressionado - verifica se foi digitação rápida (leitor de código)
        if (e.key === 'Enter') {
          // Se estava digitando rápido e tem código no buffer, é leitor de código
          if (isFastTyping && inputBuffer.length > 3) {
            e.preventDefault()
            buscarProdutoPorCodigo(inputBuffer)
            inputBuffer = ''
            isFastTyping = false
            // Limpa o campo de input que recebeu o código
            target.value = ''
            return
          }
          // Se não, deixa o Enter funcionar normalmente (humano apertou Enter)
          inputBuffer = ''
          isFastTyping = false
          return
        }

        // Acumula caracteres e detecta velocidade
        if (e.key.length === 1 && !e.ctrlKey && !e.altKey && !e.metaKey) {
          const timeDiff = now - lastKeyTime
          lastKeyTime = now

          // Se a digitação é rápida (típico de leitor), marca como fast typing
          if (timeDiff < FAST_TYPING_THRESHOLD) {
            isFastTyping = true
            inputBuffer += e.key
            // Previne o caractere de ir para o input se está em modo fast typing
            // Mas só se já detectou que é leitor (pelo menos 3 caracteres rápidos)
            if (inputBuffer.length > 3) {
              e.preventDefault()
            }
          } else {
            // Digitação lenta (humano) - reseta tudo e deixa digitar normalmente
            isFastTyping = false
            inputBuffer = ''
          }
        }
      ...
      // Fora de inputs - comportamento original
  ...
  const imprimirCupom = (tamanho?: '58mm' | '80mm' | 'A4') => {
    ...
    const config = {
      '58mm': { width: '58mm', fontSize: '10px', padding: '5px', titleSize: '14px', maxWidth: '280px' },
      '80mm': { width: '80mm', fontSize: '12px', padding: '10px', titleSize: '16px', maxWidth: '320px' },
      'A4': { width: '210mm', fontSize: '14px', padding: '20px', titleSize: '20px', maxWidth: '100%' }
    }
    ...
    const conteudo = `
      ...
      <head>
        ...
        <style>
          @page { 
            size: ${formato === 'A4' ? 'A4 portrait' : formato}; 
            margin: 0;
          }
          @media print {
            body { margin: 0; }
          ...
          .subtitle { font-size: ${c.fontSize}; margin-top: 5px; }
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <div className="flex gap-1 mr-4">
            <Button
              ...
              className={tipoImpressao === '58mm' ? 'bg-emerald-600 hover:bg-emerald-700' : ''}
            >
              58mm
            </Button>
            <Button
              ...
              className={tipoImpressao === '80mm' ? 'bg-emerald-600 hover:bg-emerald-700' : ''}
            >
              80mm
            </Button>
            <Button

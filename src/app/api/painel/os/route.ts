...
import { gerarCodigoOsUnico } from '@/lib/utils/codigo-os'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!cliente) {
      ...
      const dadosAtualizacao: { nome?: string; email?: string | null; cpf?: string | null } = {}
      if (clienteNome && cliente.nome !== clienteNome) {
        dadosAtualizacao.nome = clienteNome
      }
      if (clienteEmail !== undefined) {
        dadosAtualizacao.email = clienteEmail || cliente.email
      }
      if (clienteCpf !== undefined && clienteCpf !== cliente.cpf) {
        dadosAtualizacao.cpf = clienteCpf || null
      }
    ...
    const codigoOs = await gerarCodigoOsUnico()

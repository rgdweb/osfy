...
export function LojaPageClient({ loja, avaliacoes, mediaAvaliacoes }: LojaPageClientProps) {
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 bg-amber-50 px-3 py-1 rounded-full">
                      ...
                      <span className="font-bold text-amber-700">{mediaAvaliacoes.toFixed(1)}</span>
                      <span className="text-sm text-amber-600">({avaliacoes.length})</span>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {avaliacoes.map((avaliacao) => (
                      <div key={avaliacao.id} className="border-b border-slate-100 pb-4 last:border-0 last:pb-0">
                          ...
                          <div className="flex-1">
                            ...
                            {avaliacao.comentario && (
                              <p className="text-slate-700 text-sm leading-relaxed">
                                <Quote className="w-4 h-4 text-slate-300 inline mr-1" />
                                {avaliacao.comentario}
                              </p>
                            )}
                            {avaliacao.resposta && (
                              <div className="mt-2 ml-4 p-3 bg-slate-50 rounded-lg border-l-2 border-emerald-500">
                                <p className="text-xs text-slate-500 mb-1">Resposta da loja:</p>
                                <p className="text-sm text-slate-600">{avaliacao.resposta}</p>
                              </div>
                            )}
                          </div>
                        </div>

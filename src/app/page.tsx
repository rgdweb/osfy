...
export default function HomePage() {
  ...
  // Estados para verificação em duas etapas
  const [etapaRecuperar, setEtapaRecuperar] = useState(1) // 1 = email, 2 = verificação
  const [perguntaVerificacao, setPerguntaVerificacao] = useState('')
  const [tipoVerificacao, setTipoVerificacao] = useState('')
  const [placeholderVerificacao, setPlaceholderVerificacao] = useState('')
  const [tokenVerificacao, setTokenVerificacao] = useState('')
  const [respostaVerificacao, setRespostaVerificacao] = useState('')
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        let destino = data.tipo === 'superadmin' ? '/superadmin' : '/painel'
        
        // Se a loja está bloqueada, redirecionar para faturas
        if (data.bloqueada) {
          destino = '/painel/faturas'
          console.log('[FRONTEND] Loja bloqueada - redirecionando para faturas')
        }
  ...
  // Etapa 1: Buscar pergunta de verificação
  const handleBuscarPergunta = async (e: React.FormEvent) => {
    e.preventDefault()
    ...
    try {
      ...
      if (data.success) {
        setPerguntaVerificacao(data.pergunta)
        setTipoVerificacao(data.tipo)
        setPlaceholderVerificacao(data.placeholder)
        setTokenVerificacao(data.token)
        setEtapaRecuperar(2)
        toast.success('Email encontrado! Responda a pergunta de segurança.')
      } else {
  ...
  // Etapa 2: Validar resposta e gerar nova senha
  const handleRecuperarSenha = async (e: React.FormEvent) => {
    e.preventDefault()
    
    if (!respostaVerificacao.trim()) {
      toast.error('Responda a pergunta de segurança')
      return
    }
  ...
  const handleFecharRecuperar = (open: boolean) => {
    setRecuperarOpen(open)
    if (!open) {
      setEtapaRecuperar(1)
      setEmailRecuperar('')
      setRespostaVerificacao('')
      setSenhaGerada('')
      setPerguntaVerificacao('')
    }
  }
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <Dialog open={recuperarOpen} onOpenChange={handleFecharRecuperar}>
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {etapaRecuperar === 1 
                      ? 'Digite seu email para verificar sua identidade'
                      : 'Responda a pergunta de segurança para gerar uma nova senha'
                    }
                  </DialogDescription>
                </DialogHeader>
                ...
                {/* Etapa 1: Email */}
                {etapaRecuperar === 1 && (
                  <form onSubmit={handleBuscarPergunta} className="space-y-4 mt-4">
                    <div className="space-y-2">
                      <Label htmlFor="recuperar-email">Email cadastrado</Label>
                      <Input
                        id="recuperar-email"
                        type="email"
                        placeholder="seu@email.com"
                    ...
                    >
                      {recuperarLoading ? 'Verificando...' : 'Continuar'}
                    </Button>
                  </form>
                )}
                ...
                {etapaRecuperar === 2 && (
                  <form onSubmit={handleRecuperarSenha} className="space-y-4 mt-4">
                    <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <p className="text-sm text-blue-700 mb-1">Email: <strong>{emailRecuperar}</strong></p>
                    </div>
                    
                    <div className="space-y-2">
                      <Label htmlFor="resposta-verificacao" className="text-base font-medium">
                        {perguntaVerificacao}
                      </Label>
                      <Input
                        id="resposta-verificacao"
                        type="text"
                        placeholder={placeholderVerificacao}
                        value={respostaVerificacao}
                        onChange={(e) => setRespostaVerificacao(e.target.value)}
                        maxLength={10}
                      />
                      <p className="text-xs text-slate-500">
                        Digite apenas os números solicitados
                      </p>
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button 
                        ...
                        disabled={recuperarLoading}
                      >
                        {recuperarLoading ? 'Verificando...' : 'Gerar Nova Senha'}
                      </Button>
                    </div>
                  </form>
                )}
                ...
                {senhaGerada && (
                  <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-lg mt-4">
                    <p className="text-emerald-700 text-sm mb-2 font-medium">Nova senha gerada:</p>
                    <div className="flex items-center gap-2">
                      <code className="text-lg font-mono bg-white px-3 py-1 rounded border flex-1">
                        {senhaGerada}
                      </code>
                      <Button
                        type="button"
              ...
              {[
                {
                  step: 1,
                  title: 'Crie sua loja',
                  description: 'Cadastre sua assistência em minutos. Escolha o nome, configure os serviços e personalize com sua logo.',
                  icon: Users,
                  side: 'left'
                },
                {
          ...
          <div className="max-w-md mx-auto">
            <Card className="border-2 border-emerald-500 relative overflow-hidden">
              ...
              <CardContent className="text-center">
                <div className="mb-6">
                  <span className="text-5xl font-bold text-slate-900">R${configuracoes.sitePreco}</span>
                  <span className="text-slate-600">/mês</span>
                  <p className="text-sm text-slate-500 mt-1">ou R${configuracoes.sitePrecoAnual}/ano (2 meses grátis)</p>
                </div>

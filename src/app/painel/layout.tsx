...
import { DicasRotativas, DicasMobileBell } from '@/components/painel/DicasRotativas'
import { AssistenteVirtual } from '@/components/painel/AssistenteVirtual'

const menuItems = [
  { href: '/painel', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/painel/os', label: 'Ordens de Serviço', icon: Wrench },
  { href: '/painel/clientes', label: 'Clientes', icon: Users },
  { href: '/painel/pagamento', label: 'Pagamento', icon: CreditCard, sempreAcessivel: true },
  { href: '/painel/faturas', label: 'Faturas', icon: FileText, sempreAcessivel: true },
...
export default function PainelLayout({
        ...
        if (!data.authenticated || !data.user?.lojaId) {
          ...
          Promise.all([
            ...
            .then(([lojaData, pendenciasData, statusData]) => {
              ...
              const estaBloqueado = statusData?.loja?.bloqueado === true || statusData?.status === 'bloqueado'
              setBloqueado(estaBloqueado)
              
              // Se bloqueado, verificar se está em página permitida
              if (estaBloqueado) {
                const paginasPermitidas = ['/painel/pagamento', '/painel/faturas', '/painel/bloqueado']
  ...
  const NavLink = ({ href, label, icon: Icon, sempreAcessivel }: { href: string; label: string; icon: React.ElementType; sempreAcessivel?: boolean }) => {
    const isActive = pathname === href || (href !== '/painel' && pathname.startsWith(href))
    const desabilitado = bloqueado && !sempreAcessivel
    
    if (desabilitado) {
      return (
        <div
          className="flex items-center gap-3 px-3 py-2 rounded-lg text-slate-300 cursor-not-allowed"
          title="Acesso bloqueado - Regularize seu pagamento"
        >
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="flex grow flex-col gap-y-5 overflow-y-auto bg-white border-r border-slate-200 px-6 pb-4">
          ...
          {bloqueado && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-center">
              ...
              <p className="text-xs text-red-500">Regularize para desbloquear</p>
            </div>
          )}
          ...
          <nav className="flex flex-1 flex-col">
            ...
            {/* Dicas Rotativas - Desktop */}
            {!bloqueado && (
              <div className="hidden lg:block mt-auto">
                <DicasRotativas variant="sidebar" />
              </div>
            )}
      ...
      {/* Assistente Virtual - Robô Flutuante - só mostra se não bloqueado */}
      {!bloqueado && <AssistenteVirtual />}
    </div>
  )
}

...
interface OSDetailPageProps {
  os: OrdemServico & {
    ...
    loja: { id: string; slug: string; nome: string; telefone?: string; endereco?: string; cidade?: string; estado?: string; logo?: string }
...
${os.loja.nome}`
  ...
  // Gerar pagamento online para o cliente (Mercado Pago - não exige CPF)
  const handleGerarPagamento = async () => {
    setGerandoPagamento(true)
    
    try {
      // Gerar link de pagamento (preferência Mercado Pago)
      ...
      if (data.success) {
        ...
        toast.success('Link de pagamento gerado! O cliente pode acessar a página pública para pagar.')
      } else {
                  ...
                  {(os.mpPaymentId || os.mpPreferenceId || pagamentoGerado) ? (
                    ...
                    <>
                      <p className="text-blue-600 text-sm mb-3">
                        Gere um link de pagamento Mercado Pago. O cliente poderá escolher entre PIX, Cartão ou Boleto.
                      </p>

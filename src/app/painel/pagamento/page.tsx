...
export default function PagamentoPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Tem fatura pendente mas sem dados de pagamento - mostrar botão para gerar */}
      {temPagamentoPendente && status?.pagamento.fatura && !status.pagamento.fatura.linkPagamento && (
        <Card className="border-amber-200 overflow-hidden shadow-lg">
              ...
              onClick={async () => {
                ...
                try {
                  const res = await fetch('/api/cobranca/gerar', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ faturaId: status?.pagamento.fatura?.id, formaPagamento: 'link' })
                  })
